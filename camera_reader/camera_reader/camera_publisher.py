...
class CameraPublisher(Node):
    def __init__(self):
        super().__init__('usd_camera_publisher')
        ...
        if not self.cap.isOpened():
            self.get_logger().info(f'Could not open image device with ID:{self.camera_id}')
            raise SystemExit
    ...
    def get_image_frame(self):
        ...
        if ret:
            ...
            gray_frame =cv2.cvtColor(resized_frame, cv2.COLOR_BAYER_BG2BGR)
    ...
    def image_elaboration(self):
        if self.img_ready ==True:
            ...
            cv2.putText(frame, 'Device: '+self.camera_id, position_device,font,font_scale,font_color,font_thickness)
